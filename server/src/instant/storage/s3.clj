...
;; Makes list-objects work with docker container.
;; http://localhost:9000 throws error. http://minio:9000 works
(def credentials {:access-key    (config/s3-access-key)
                  :secret-key    (config/s3-secret-key)
                  :endpoint      (config/s3-endpoint)
                  :client-config {:path-style-access-enabled true}})

;; Revert to localhost:9000 for files, see above
(def credentialsSignedUrl {:access-key    (config/s3-access-key)
                           :secret-key    (config/s3-secret-key)
                           :endpoint      (config/s3-signed-url-endpoint)
                           :client-config {:path-style-access-enabled true}})
...
(defn delete-object
  ...
  ([bucket-name object-key]
   (s3/delete-object credentials {:bucket-name bucket-name
                                  :key object-key})))

(defn delete-objects
  ([object-keys] (delete-objects default-bucket object-keys))
  ([bucket-name object-keys]
   (s3/delete-objects credentials {:bucket-name bucket-name
...
(defn list-app-objects
  ...
  ([bucket-name prefix]
   (s3/list-objects credentials {:bucket-name bucket-name :prefix prefix})))
...
(defn signed-upload-url
  ...
  ([bucket-name object-key]
   (s3/generate-presigned-url credentialsSignedUrl {:method :put
...
(defn signed-download-url
  ...
  ([bucket-name object-key expiration]
   (s3/generate-presigned-url credentialsSignedUrl {:method :get
                                                    :bucket-name bucket-name
                                                    :key object-key
